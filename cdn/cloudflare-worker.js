...
function parseImageParams(url) {
  ...
  const quality = parseInt(url.searchParams.get('q')) || CONFIG.IMAGE.DEFAULT_QUALITY;
  const format = url.searchParams.get('f') || CONFIG.IMAGE.DEFAULT_FORMAT;

  return {
    width: width ? parseInt(width) : undefined,
    height: height ? parseInt(height) : undefined,
    quality: Math.min(Math.max(quality, CONFIG.IMAGE.MIN_QUALITY), CONFIG.IMAGE.MAX_QUALITY),
    format: CONFIG.IMAGE.SUPPORTED_FORMATS.includes(format) ? format : CONFIG.IMAGE.DEFAULT_FORMAT,
  };
}
