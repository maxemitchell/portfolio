...
const PhotoCollectionPreview = ({ slug, title, fluid }) => {
  return (
    <Link
      className="w-5/12 md:w-2/5 mx-4 mb-3 group"
      to={`/photo_collections/${slug}/`}
    >
      <h3 className="w-full text-xl md:text-2xl mb-1 font-medium text-themeBlue group-hover:text-themeRed duration-500">
        {title}
      </h3>
      <Img
