  ...
  Sketch(p) {
    ...
    p.draw = () => {
      ...
      for (let i = 0; i < NUM_RINGS; i++) {
        times[i] =
          times[i] +
          p.constrain(0.012 * beatLevels[i], 0.0009, 0.012) *
            (-2 * beatState + 1)
      }
      ...
      for (let i = 1; i < NUM_RINGS + 1; i++) {
        ...
        for (let angleIter = 0; angleIter < ringDotCount / 2; angleIter++) {
          let angle1 =
            times[i - 1] * (-2 * (i % 2) + 1) +
            angleIter * (p.TWO_PI / ringDotCount)
          let angle2 =
            times[i - 1] * (-2 * (i % 2) + 1) -
            angleIter * (p.TWO_PI / ringDotCount)
          sizes[curDot] = sizes[curDot] ** (i / NUM_RINGS + 0.9) // Scale the sizes exponentially so that the edges are larger
          ...
          if (angleIter + 1 == ringDotCount / 2) {
            let angle =
              times[i - 1] * (-2 * (i % 2) + 1) +
              (angleIter + 1) * (p.TWO_PI / ringDotCount)
            x = r * p.sin(angle)
            y = r * p.cos(angle)
            p.ellipse(x, y, sizes[curDot])
          }
          ...
          <div className="flex w-full flex-wrap max-w-sm lg:w-1/2 mb-4 lg:mx-6 lg:justify-start">
            ...
            <div className="flex w-full boxshadow-3d-right mt-4 lg:mt-8">
              <p className="w-full text-sm md:text-md lg:text-lg font-extralight font-manrope m-4">
                ...
                You can cycle the color palettes with <b>space bar</b>.
              </p>
            </div>
          </div>
        </div>
