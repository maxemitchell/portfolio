  ...
  Sketch(p) {
    ...
    class AveragedSquare {
      ...
      draw() {
        p.fill(this.color)
        p.noStroke()
        p.square(this.x, this.y, this.l + 0.5, 0)
      }
    }
    ...
    class Squares {
      constructor() {
        ...
        for (let i = 0; i < this.NUM_SQUARES; i++) {
          ...
          let x =
            i * length +
            this.SPACING * i * lengthMargin +
            (lengthMargin * this.SPACING) / 2

          for (let j = 0; j < this.NUM_SQUARES; j++) {
            let isSource =
              p.random(this.NUM_SQUARES * this.NUM_SQUARES) < this.NUM_SOURCES
            let y =
              j * height +
              this.SPACING * j * heightMargin +
              (heightMargin * this.SPACING) / 2
        ...
        for (let i = 0; i < this.squares.length; i++) {
          for (let j = 0; j < this.squares[i].length; j++) {
            ...
            if (curSquare.isSource) {
              curSquare.hue = (curSquare.hue + 0.6) % 360
            } else {
              let targetHueX = 0
              let targetHueY = 0
              let neighborHue
      ...
      getHue(row, col) {
        if (
          row < 0 ||
          col < 0 ||
          row == this.NUM_SQUARES ||
          col == this.NUM_SQUARES
        ) {
          return -1
        } else {
          ...
          <div className="flex w-full flex-wrap max-w-sm lg:w-1/2 mb-4 lg:mx-6 lg:justify-start">
            ...
            <div className="flex w-full boxshadow-3d-right mt-4 lg:mt-8 mb-4 ">
              <p className="w-full text-sm md:text-md lg:text-lg font-extralight font-manrope m-4">
                ...
                on reddit, I created some sort of naturally changing gradient.
                It has a nice pastel look to it, and works with a handful of
                source points that rotate the colorwheel while the rest of the
                points take the average value of their neighbors.
              </p>
            </div>
